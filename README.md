...
| Rate–distortion | Illustrative only | `rate_stats()` FP32 + `quantized_rate_stats()` / STE (+ **`LearnedQuantAffine`**) + **factorized Laplace** `-log2 p`; see [ENTROPY-CODING-NOTES.md](./docs/ENTROPY-CODING-NOTES.md) |
| Bottleneck train sketch | CPU dry-run + STE + learned scales + entropy | [`bottleneck_train_sketch.py`](./bottleneck_train_sketch.py) (`--ste-quant`, `--learned-quant-scales`, `--entropy-rate`) + [docs/TRAINING-SKETCH.md](./docs/TRAINING-SKETCH.md) |

Default illustrative rate at 512²: **256×4 B = 1024 B** → **~0.031 bpp** before generative decode (not a trained RD curve). Uniform 8-bit symbols on the same dim sketch **~0.0078 bpp**. Factorized Laplace expected `-log2 p` is the differentiable rate proxy (`--entropy-rate`) — still not ANS. `--learned-quant-scales` adapts per-dim STE ranges.
